...
// ResponseTimeRaw retorna o tempo de resposta médio em texto simples
func ResponseTimeRaw(c *fiber.Ctx) error {
    duration := c.Params("duration")
    var from time.Time

    // Define o intervalo de tempo com base na duração fornecida
    switch duration {
    case "30d":
        from = time.Now().Add(-30 * 24 * time.Hour)
    case "7d":
    ...
    case "24h":
        from = time.Now().Add(-24 * time.Hour)
    case "1h":
        from = time.Now().Add(-2 * time.Hour) // Ajuste necessário para métricas armazenadas por hora
    default:
        return c.Status(400).SendString("Durations supported: 30d, 7d, 24h, 1h")
    }
    ...
    // Obtém o tempo de resposta médio a partir do armazenamento
    ...
    // Retorna o tempo de resposta médio como texto simples
    return c.Status(200).Send([]byte(fmt.Sprintf("%d", averageResponseTime)))
